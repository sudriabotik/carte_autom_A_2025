    ...
    while(1){
        if(SYS_JACK){
            ...
            if (COULEUR == VIOLET){ printf("Strat violet");
                ...
                passe_part(1850, 240, MARCHE_AVANT, 25, FIN_TRAJECTOIRE);
                //on ferme le bras 
                ...
                rejoindre(2180,235, MARCHE_AVANT, 40);    
                //2210
                //ON A LE GOLDENIUM
                ...
                //RISQUE DE COLLISION
                //EVITEMENT_ADV.actif = ON;
                //EVITEMENT_ADV.sens = MARCHE_AVANT; //LIGNE pas testée
                //va vers la balance
                ...
                delay_ms(500);
                //recule avec le bleu
                EVITEMENT_ADV.actif = ON; //pas testées
                EVITEMENT_ADV.sens = MARCHE_ARRIERE; //LIGNE QUE J AVAIS OUBLIE ! PERMET DE REGARDER DERRIERE POUR L EVITEMENT !! risque de soucis
                rejoindre(780,1200,MARCHE_ARRIERE,25);
                //monte le brasavec le bleu
                angle_AX12(BRAS_POMPE,500,100,SANS_ATTENTE);
                ...
                rejoindre(1270,1435,MARCHE_AVANT,25);
                envoit_pwm(MOTEUR_X,0); //lache le bleu
                
                //prend le vert (pas testé)
               
                //RECUPERATION VERT BALANCE
                //quitte la balance en marche arriere
                EVITEMENT_ADV.actif = ON; //pas testées
                EVITEMENT_ADV.sens = MARCHE_ARRIERE; //LIGNE QUE J AVAIS OUBLIE ! PERMET DE REGARDER DERRIERE POUR L EVITEMENT !! risque de soucis
                rejoindre(1280, 1200, MARCHE_ARRIERE, 35);
                ...
                passe_part(1280, 1200,MARCHE_AVANT,50,DEBUT_TRAJECTOIRE);
                EVITEMENT_ADV.actif = ON; //pas testée la ligne suivante
                 EVITEMENT_ADV.sens = MARCHE_AVANT; //LIGNE QUE J AVAIS OUBLIE ! PERMET DE REGARDER DERRIERE POUR L EVITEMENT !! risque de soucis
                //on oriente la pompe
                angle_AX12(BRAS_POMPE,0,100,SANS_ATTENTE);
                //on va vers le bleu (selonles x)
                passe_part(980,1200,MARCHE_AVANT,50,FIN_TRAJECTOIRE);
                EVITEMENT_ADV.actif = OFF;
                //s'oriente vers le vert
                orienter(90.,50);
                rejoindre(980,1385,MARCHE_AVANT,10);
                //pompe allumé recup vert
                envoit_pwm(MOTEUR_X,40);
                //va droit dans le mur
                rejoindre(980,1410,MARCHE_AVANT,10);
                delay_ms(500);
                
                //recule avec le vert
                rejoindre(980,1200,MARCHE_ARRIERE,25);
                //monte le brasavec le vert
                angle_AX12(BRAS_POMPE,500,100,SANS_ATTENTE);
                //s'oriente vers la balance
                orienter(0.,50);
                //va vers la balance
            ...
            //Chemin vert le goldenium
                ...
                //on avance en poussant le palet
                passe_part(1850, 180, MARCHE_AVANT, 25, FIN_TRAJECTOIRE);
                //on ferme le bras 
                ...
                //allumer pompe
                envoit_pwm(MOTEUR_X,40);
                rejoindre(2260,135, MARCHE_AVANT, 40); 
                ...
                //s'oriente vers la balance
                EVITEMENT_ADV.actif = OFF; //pas testées
                ...
                angle_AX12(BRAS_POMPE,0,100,SANS_ATTENTE);
                //avance vers le vert
                passe_part(1030,1200,MARCHE_AVANT,50,FIN_TRAJECTOIRE);
                EVITEMENT_ADV.actif = OFF;
                //s'oriente vers le vert
                orienter(90.,50);
                //avance droit dans le mur
                rejoindre(1030,1385,MARCHE_AVANT,10);
                //on enclenche la pompe
                envoit_pwm(MOTEUR_X,40);
                rejoindre(1030,1410,MARCHE_AVANT,10);
                delay_ms(500);
                //recule avec le vert
                rejoindre(1030,1200,MARCHE_ARRIERE,25);
                //monte le bleu
                angle_AX12(BRAS_POMPE,300,100,SANS_ATTENTE);
                //s'oriente vers la balance
                orienter(0.,50);
